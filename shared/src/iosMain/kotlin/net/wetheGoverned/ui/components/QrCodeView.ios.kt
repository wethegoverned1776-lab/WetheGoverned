...
actual fun QrCodeView(
    ...
    val matrix = remember(data) {
        try {
            ...
            Array(size) { row -> BooleanArray(size) { col -> rawData[row][col].dark } }
        } catch (e: Exception) {
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            for (row in 0 until matrix.size) {
                for (col in 0 until matrix.size) {
                    if (matrix[row][col]) {
                        drawRect(color = color, topLeft = Offset(col * cellSize, row * cellSize), size = Size(cellSize + 1f, cellSize + 1f))
                    }
                }
            }
        }
